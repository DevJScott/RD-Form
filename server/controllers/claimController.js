...
const createClaim = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { formData, isDraft = true, currentStep = 1, claimTitle } = req.body;

    const result = await req.app.locals.db.query(
      `INSERT INTO claims (user_id, claim_title, form_data, is_draft, current_step, last_saved_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       RETURNING *`,
      [userId, claimTitle || 'New Claim', formData || {}, isDraft, currentStep]
    );
  ...
  try {
    ...
    const result = await req.app.locals.db.query(
      `UPDATE claims 
       ...
       RETURNING *`,
      [formData, isDraft, currentStep, claimTitle, id, userId]
    );
